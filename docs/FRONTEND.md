...
- `handleForgotPassword` (build 7) abre `Config.FORGOT_PASSWORD_URL` → `apps/web` `/auth/forgot-password` con el mismo patrón (`openLinkInBrowser`); enlace "¿Olvidaste tu contraseña?" debajo de la fila de contraseña, clave `loginScreen:forgotPasswordLink` en los 7 idiomas. Variable: `EXPO_PUBLIC_FORGOT_PASSWORD_URL` en los perfiles `preview` y `production` de `eas.json` (`development` hereda de `production`), fallback `https://eliteforge.tech/auth/forgot-password`; en dev usa el mismo host que la API (`getDevApiHost`).
...
- **`LoginScreen`:** enlace "¿Olvidaste tu contraseña?" debajo de la fila contraseña + Iniciar sesión, alineado a la derecha (mismo estilo naranja subrayado de `LinkText`). Abre en el navegador `Config.FORGOT_PASSWORD_URL` (patrón de "Crear cuenta"): la recuperación vive en la web y manda el enlace por correo; la app no tiene formulario propio. Config nueva en `config.dev.ts` / `config.prod.ts` y `EXPO_PUBLIC_FORGOT_PASSWORD_URL` en `eas.json`.
- **`ProfileEditScreen` → sección "Seguridad"** (debajo de "Posición favorita", antes de Guardar perfil): botón *Cambiar contraseña* (variante `outline`) que abre `ChangePasswordModal` (`screens/profile/components/`, misma hoja inferior que `GroupAddMemberModal`) con tres campos `Input` con ojo de mostrar/ocultar: actual, nueva, repetir. Validación local antes de pegarle al backend (repetir ≠ nueva → `changePasswordMismatch`; 8–72 con letra y número → `changePasswordRules`, las mismas reglas que `ChangePasswordDto`) porque la ruta tiene límite de 5 intentos por minuto. Errores: 401 → "La contraseña actual no es correcta" (`api.changePassword` devuelve `unauthorized`; el monitor de 401 excluye `auth/*`, así que **no** cierra la sesión); cualquier otro → `changePasswordError`. Éxito: alerta "Contraseña actualizada — tu sesión en este teléfono sigue abierta, las demás se cerraron".
- **Qué pasa con la sesión al cambiar la contraseña (verificado):** el backend marca `passwordChangedAt` y el gateway rechaza todo JWT anterior, **incluido el que la app acaba de usar** — sin más, el siguiente request daba 401 y el monitor cerraba la sesión. Arreglo: `POST auth/password/change` devuelve `{ ok, accessToken }` con un JWT nuevo firmado después del cambio, y `handleChangePassword` lo guarda con `setAuthToken` (MMKV, síncrono) **antes** de cualquier otro request. Efecto colateral esperado: los `useEffect` que dependen de `authToken` en `AuthContext` corren otra vez (recarga del avatar y re-registro del push token, ambos idempotentes). Las otras sesiones del usuario quedan revocadas. Detalle del lado servidor (comparación al segundo, `forget` de la caché) en [BACKEND.md](./BACKEND.md#build-7-2026-09-11-aviso-de-copa-a3-cambio-de-contraseña-sin-perder-la-sesión-limpieza).
- **A3 — push "Nueva Copa Elite Forge":** `PushType` += `tournament_announced`, `PushScreen` += `TournamentDetail`, `PUSH_SCREENS.TournamentDetail` exige `params.tournamentId` (sin él no navega) → abre el detalle público de la Copa, donde el líder inscribe a su grupo. Test en `pushNavigation.test.ts`. Solo lista blanca: la pantalla ya existía.
- **Limpieza:** borrados `api.addGroupMember` y las 7 claves `groupsScreen:addMember*` / `addingMember` en los 7 idiomas (la ruta del backend ya no existe). `GroupAddMemberModal` conserva su nombre de archivo y sigue usando las claves `invite*`.
- **i18n:** 16 claves nuevas en los 7 idiomas registrados (`ar`, `en`, `es`, `fr`, `hi`, `ja`, `ko`): `loginScreen:forgotPasswordLink` y 15 bajo `profileScreen` (`securityTitle`, `securityHint`, `changePasswordButton`, `changePasswordTitle`, `changePasswordCurrent`, `changePasswordNew`, `changePasswordConfirm`, `changePasswordSubmit`, `changingPassword`, `changePasswordRules`, `changePasswordMismatch`, `changePasswordWrongCurrent`, `changePasswordError`, `changePasswordSuccessTitle`, `changePasswordSuccessMessage`). Paridad verificada por `test/i18n.test.ts`. Nota: la app registra **7** idiomas en `i18n/index.ts`, no 8.
- **QA en dispositivo (build 7):** tocar "¿Olvidaste tu contraseña?" abre el navegador en la web; cambiar la contraseña con la actual mal (mensaje, sin cierre de sesión), con repetir distinto, con una débil, y con todo bien → seguir navegando sin que la app cierre la sesión; en un segundo teléfono con la misma cuenta, el próximo request cierra la sesión; con un tournament elite_forge creado por el admin, el push llega a un Jugador y el tap abre el detalle de la Copa.
