...
**Aura y energía.** Filtro SVG `#ef-aura-filter` aplicado a un `<use>` de la silueta, con región amplia (`x -90 % / w 280 %`) para que nada se recorte. Cuatro capas de afuera hacia adentro: bruma lejana cian (dilate 28 + blur 34, deformada con ruido ancho `scale 140`, 34 %), niebla naranja (dilate 18 + blur 24, ruido ancho con canales invertidos `scale 96`, 46 %), halo cian pegado al cuerpo (dilate 12 + blur 16, ruido fino `scale 64`, 68 %) y contorno (dilate 2,5 + blur 1,4, cian al 92 %). Los dos colores de marca se entremezclan porque cada capa usa un ruido distinto. Los tres `<svg>` de cada pose llevan `overflow-visible`: el halo y los trazos salen del viewBox 400×440 sin cortarse (solo recorta el `overflow-hidden` del hero). Los trazos de energía alternan cian y naranja (≈ 1 de cada 3) y los wisps también, con cuatro más detrás del cuerpo, del lado de la estela. Debajo, la capa de energía (`.ef-energy-layer`): trazos quebrados que salen del cuerpo hacia atrás (sentido contrario al movimiento, `trail` por pose) generados de forma determinista (`energyStreaks`, LCG con semilla por pose para que servidor y cliente coincidan) desde anclas en cabeza, hombros, codos, cadera, rodillas y tobillo, con el filtro `#ef-streak-glow` (blur ancho + núcleo nítido) y una luz de suelo bajo los pies (`#ef-ground-blur`). La capa deriva apenas hacia atrás con `ef-energy-drift` (3,6 s). La capa del aura respira con `ef-breathe` (opacity 0,72 ↔ 1 y `scale` 1 ↔ 1,018 en 4,2 s, ease-in-out, nunca parpadea). Cinco "wisps" (`.ef-wisp`, `<span>` con `radial-gradient` + `blur(6px)`) nacen sobre cabeza y hombros, ascienden, se abren y se disipan (`ef-vapor`, 5,2 s, desfasados). En la pose 3, un `radialGradient` naranja detrás del balón, tres estelas y seis partículas (`.ef-spark-impact`, `ef-impact-spark`) marcan el impacto.
